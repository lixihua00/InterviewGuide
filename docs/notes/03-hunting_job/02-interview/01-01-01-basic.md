...
- 当把指针作为参数进行传递时，也是将实参的一个拷贝传递给形参，两者指向的地址相同，但不是同一个变量，在函数中改变这个变量的指向不影响实参，而引用却可以。（第一次看到这句话确实感到疑惑：明明是指向相同的地址，为什么在函数内部改变指针的值不会影响实参？这里要区分两个点：是通过指针改变所指向的内存的值，还是改变指针的指向，如果是改变内存的值，那么是可以的；如果是在函数内改变指针的指向不会影响实参，相当于有两个指针，只不过这两个指针指向相同的内存，改变其中一个指针的指向，并不会影响另一个指针的指向。）
